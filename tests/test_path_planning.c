...
static EcuMap make_oval(int n, float length, float half_w)
{
    EcuMap t;
    memset(&t, 0, sizeof(t));
    if (n > ECU_MAX_CONES) n = ECU_MAX_CONES;
    for (int i = 0; i < n; i++) {
...
static EcuMap make_ring(int n, float radius, float half_w)
{
    EcuMap t;
    memset(&t, 0, sizeof(t));
    if (n > ECU_MAX_CONES) n = ECU_MAX_CONES;
...
static float mean_radius_offset(const EcuMap *t, float centre_radius)
{
    float acc = 0.0f;
...
{
    EcuMap t = make_oval(20, 40.0f, 2.0f);
    path_plan(&t);
    ASSERT(t.count >= 4);
}
...
{
    float half_w = 2.5f;
    EcuMap t      = make_oval(30, 60.0f, half_w);
    path_plan(&t);
...
{
    EcuMap t = make_oval(25, 50.0f, 2.0f);
    path_plan(&t);
...
{
    EcuMap t = make_oval(20, 40.0f, 3.0f);
    path_plan(&t);
...
{
    EcuMap t = make_oval(ECU_MAX_CONES, 100.0f, 2.0f);
    path_plan(&t);
    ASSERT(t.count > 0);
    ASSERT(t.count <= ECU_MAX_WAYPOINTS);
}
...
static void test_ring_inside_corridor(void)
{
    ...
    EcuMap t      = make_ring(60, radius, half_w);
    path_plan(&t);
...
{
    EcuMap t1 = make_oval(20, 40.0f, 2.0f);
    EcuMap t2 = t1;
