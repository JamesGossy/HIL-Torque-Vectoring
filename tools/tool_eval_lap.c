...
int main(void)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        float abx=bx-ax, aby=by-ay, bcx=cx-bx, bcy=cy-by, cax=ax-cx, cay=ay-cy;
        float ab=sqrtf(abx*abx+aby*aby), bc=sqrtf(bcx*bcx+bcy*bcy), ca=sqrtf(cax*cax+cay*cay);
        float crs=fabsf(abx*bcy-aby*bcx);
        kappa[i] = (ab<0.01f||bc<0.01f||ca<0.01f||crs<1e-6f) ? 0.0f : 2.0f*crs/(ab*bc*ca);
    }
    ...
    for (int tick = 1; tick <= max_ticks; tick++) {
        ...
        sum_cte += cte; cnt++;
        if (cte > worst_cte) { worst_cte = cte; worst_idx = ni; }
        ...
        if (moved_away && track.lap_count > start_lap && lap_done_tick < 0)
            lap_done_tick = tick;
    ...
    printf("worst CTE:       %.3f m  (near waypoint %d, kappa=%.3f 1/m, R=%.1f m)\n",
           worst_cte, worst_idx, worst_idx>=0?kappa[worst_idx]:0.0f,
    ...
    printf("RESULT mean_cte=%.3f worst_cte=%.3f worst_cte_sharp=%.3f "
           ...
           (float)(sum_cte/cnt), worst_cte, worst_cte_sharp,
           sharp_violations, offtrack_ticks, track.lap_count - start_lap,
